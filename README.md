...
| Client (CometBFT) and application version adoption           | aggregate share only; a version run by fewer than k=5 nodes is folded into "other"; the client panel is withheld below 20 observed nodes, the application panel below 10 responders | never joinable with country, ASN, or endpoint; the application version is known for nodes that answer RPC only, public endpoints anyone can ask directly, so k is the guard that matters there |
